...
rtrrt_ns::Node rtrrt::steer(rtrrt_ns::Node x_s){
	...
	kdres* nearest_node = kd_nearest3(kdTree_, x_s.state.x, x_s.state.y, 0);
	if (kd_res_size(nearest_node) <= 0)
	{
		kd_res_free(nearest_node);
		return x_new;
	}
	rtrrt_ns::Node* nearestNode = (rtrrt_ns::Node*)kd_res_item_data(nearest_node);
	kd_res_free(nearest_node);
...
bool rtrrt::addNodeEdge(rtrrt_ns::Node x_new, rtrrt_ns::Node *x_parent){
	rtrrt_ns::Node* new_node = new rtrrt_ns::Node;
	...
	if(checkNewNode(x_new)){
		...
		if(dis_new_parent < minNodeDistance){
			delete new_node;
			return false;
		}
		x_new.state.z = getElevation(x_new.state.x, x_new.state.y, full_grid_map)+0.1;
		rtrrt_ns::Edge *parent_to_new = new rtrrt_ns::Edge;
		...
		if(checkEdge(*parent_to_new)){
			...
			// Add to tree
			kd_insert3(kdTree_, new_node->state.x, new_node->state.y, 0.0, new_node);
			...
			if(util::getOrientation(x_parent->obstacle_orientation) > obstacleSectorNumThre){
				...
				if(find_result == obstacle_node_set.end() && x_parent->nodeState == NodeState::LEAF){	// No found
					...
					kd_insert3(obstacleTree_, x_parent->state.x, x_parent->state.y, 0.0, x_parent);
					...
					if(x_parent->nodeState != NodeState::ROOT){
						...
						if(x_parent->parent->children.size() == 0 && x_parent->parent->nodeState!=NodeState::ROOT){
					...
					kdres* near_nodes = getNeiborNodes(x_parent->state.x, x_parent->state.y, inflationRadius);
					double pos[3];
					...
					if (near_nodes_size <= 0)
					{
						...
						while(!kd_res_end(near_nodes)){
							...
							queue<rtrrt_ns::Node*> q_update;
							q_update.push(node);
							while(!q_update.empty()){
								rtrrt_ns::Node *n = q_update.front();
								q_update.pop();
...
void rtrrt::reselectParent(rtrrt_ns::Node *x_new){

	float dis_min = x_new->disToRoot;
	rtrrt_ns::Node *best_neibor = NULL;
	rtrrt_ns::Edge *best_neibo_edge = new rtrrt_ns::Edge;
	// Find near nodes using kd-tree within a range
	kdres* neibor_nodes = kd_nearest_range3(kdTree_, x_new->state.x, x_new->state.y, 0.0, neiborRangeThre);
	double pos[3];
	int neibor_nodes_size = kd_res_size(neibor_nodes);
	if (neibor_nodes_size <= 1)
	{
		...
		while(!kd_res_end(neibor_nodes)){
			rtrrt_ns::Node* neibor = (rtrrt_ns::Node*)kd_res_item(neibor_nodes, pos);
			// Except OBSTACLE nodes
			if(neibor->nodeState == NodeState::OBSTACLE){
				kd_res_next(neibor_nodes);
				continue;
			}
		...
		if(x_new->parent != best_neibor && best_neibor != NULL){
			...
			if(x_new->parent->nodeState!=NodeState::LEAF && x_new->parent->children.size() ==0){
...
void rtrrt::rewireRandomNode(){
	...
	while(q_rewire_from_rand.size()!=0 && (ros::Time::now().toSec() - timeKeeper) < allowRewiringFromRand){
		...
		Node *x_rand = q_rewire_from_rand.front();
		q_rewire_from_rand.pop();
		// Find near nodes using kd-tree within a range
		kdres* neibor_nodes = kd_nearest_range3(kdTree_, x_rand->state.x, x_rand->state.y, 0.0, neiborRangeThre);
		double pos[3];
		int neibor_nodes_size = kd_res_size(neibor_nodes);
		if (neibor_nodes_size <= 1)
		{
			...
			while(!kd_res_end(neibor_nodes)){
				...
				rtrrt_ns::Edge *rand_neibor_edge = new rtrrt_ns::Edge;
				...
				if(checkEdge(*rand_neibor_edge)){
					...
					if(dis < neibor->disToRoot && neibor->parent != x_rand){
						...
						// Delete specific edge
						// auto ele = find(edge_set.begin(), edge_set.end(), *(neibor->endEdge));
						// if(ele != edge_set.end()){
						// 	edge_set.erase(ele);
						// }
...
void rtrrt::rewireFromRoot(){
	unordered_set<rtrrt_ns::Node*> close_node_set;
	...
	while(!q_rewire_from_root.empty() && (ros::Time::now().toSec() - timeKeeper) < allowRewiringFromRoot){
		// Take the front of RTRRTstarrewireRand to rewire
		Node* x_r = q_rewire_from_root.front();
		q_rewire_from_root.pop();
		// Find near nodes using kd-tree within a range
		kdres* neibor_nodes = kd_nearest_range3(kdTree_, x_r->state.x, x_r->state.y, 0.0, neiborRangeThre);
		double pos[3];
		int neibor_nodes_size = kd_res_size(neibor_nodes);
		if (neibor_nodes_size <= 1)
		{
			...
			while(!kd_res_end(neibor_nodes)){
				...
				rtrrt_ns::Edge *xr_neibor_edge = new rtrrt_ns::Edge;
...
void rtrrt::changeRoot(rtrrt_ns::Node *new_root){
	...
	rtrrt_ns::Edge *new_root_edge = new rtrrt_ns::Edge;
	...
	queue<rtrrt_ns::Node*> q_old_root_children;
	q_old_root_children.push(root);
	while(!q_old_root_children.empty()){
		rtrrt_ns::Node *old_child = q_old_root_children.front();
	...
	queue<rtrrt_ns::Node*> q_new_root_children;
	q_new_root_children.push(new_root);
	while(!q_new_root_children.empty()){
		rtrrt_ns::Node *child = q_new_root_children.front();
...
rtrrt_ns::Node* rtrrt::findTarget(){
	if(newGoalReceived){
		...
		kdres* candidate_targets = kd_nearest_range3(kdTree_, goal.position.x, goal.position.y, 0.0, targetTolerance);
		double pos[3];
		int candidate_targets_size = kd_res_size(candidate_targets);
		if (candidate_targets_size <= 0)
		{
			...
			while(!kd_res_end(candidate_targets)){
				rtrrt_ns::Node *node = (rtrrt_ns::Node*)kd_res_item(candidate_targets, pos);
				// Except OBSTACLE nodes
			...
			kd_res_rewind(candidate_targets);
			// Iterate all candidate_targets to find best node
			while(!kd_res_end(candidate_targets)){
				rtrrt_ns::Node *node = (rtrrt_ns::Node*)kd_res_item(candidate_targets, pos);
				// Except OBSTACLE nodes
				if(node->nodeState == NodeState::OBSTACLE){
...
void rtrrt::executeTreeUpdate(){
	...
	if(tree.empty() && node_set.size() == 0 && updatedPose){
		// rtrrt_ns::Node root_node(robot_pose.x, robot_pose.y, robot_pose.z, 0.0, 0,0, NodeState::ROOT);
		rtrrt_ns::Node* root_node = new rtrrt_ns::Node;
	...
	kdres* nearest_node = kd_nearest3(kdTree_, newNode.state.x, newNode.state.y, 0);
	if (kd_res_size(nearest_node) <= 0)
	{
		kd_res_free(nearest_node);
		return;
    ...
    while(pt != leaf_nodes.end()){
		...
		if(full_grid_map.isInside(node_posi)){
			...
			if(dis > frontierDisThre){
				kdres* neibor_nodes = kd_nearest_range3(kdTree_, (*pt)->state.x, (*pt)->state.y, 0.0, exploredAreaRadius);
				int neibor_nodes_size = kd_res_size(neibor_nodes);
				// if(neibor_nodes_size < min_size){
				// 	min_size = neibor_nodes_size;
				// }
				// if(neibor_nodes_size > max_size){
				// 	max_size = neibor_nodes_size;
				// }
...
kdres* rtrrt::getNeiborNodes(float point_x, float point_y, float range){
    return kd_nearest_range3(kdTree_, point_x, point_y, 0.0, range);
}

rtrrt_ns::Node* rtrrt::getNearestNode(float point_x, float point_y){
	kdres* nearest_node = kd_nearest3(kdTree_, point_x, point_y, 0);
	if (kd_res_size(nearest_node) <= 0)
	{
		kd_res_free(nearest_node);
		return NULL;
...
rtrrt_ns::Node* rtrrt::getObstacleNode(float point_x, float point_y){
	kdres* nearest_node = kd_nearest3(obstacleTree_, point_x, point_y, 0);
	if (kd_res_size(nearest_node) <= 0)
	{
		kd_res_free(nearest_node);
		return NULL;
...
void rtrrt::updateOrientationArray(rtrrt_ns::Node *new_node, rtrrt_ns::Node *rrt_node){
	...
	float deltAngle = atan2(deltY, deltX) * 180/PI;	// Degree form
...
void rtrrt::visualizationCb(const ros::TimerEvent&){
